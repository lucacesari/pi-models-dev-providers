...
import { fetchAvailableProviders, ProviderItem } from "../models.loader";
import { Container, SettingItem, SettingsList, Text } from "@earendil-works/pi-tui";
...
export const selectModelsDevProviders = (
    ...
    pipe(Effect.gen(function* () {
            ...
            const providers: ReadonlyArray<ProviderItem> = yield* pipe(
                ...
                Effect.catchAll(error => Effect.fail(new ModelsDevPickerError({
            ...
            const enabledProviders = pipe(
                providers,
                Array.map(({id}) => [id, HashSet.has(currentSelections, id)] as const),
                HashMap.fromIterable
            )

            const newProviders = yield* pickUserSelections(ctx, providers, enabledProviders)
...
const pickUserSelections = (ctx: ExtensionCommandContext, providers: ReadonlyArray<ProviderItem>, enabled: HashMap.HashMap<string, boolean>) => {
    ...
    const items: SettingItem[] = pipe(
        ...
        Array.map(({id, name}) => ({
            id,
            label: name ?? id,
            currentValue: pipe(HashMap.get(enabled, id), Option.map(b => b ? 'on' : 'off'), Option.getOrElse(constant('off'))),
            values: ['on', 'off']
        }))
    )
